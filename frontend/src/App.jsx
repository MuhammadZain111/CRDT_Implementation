...
function App() {
  ...
  const [editorValue, setEditorValue] = useState(`function App() {
...
const handleJoin = (e) => {
  ...
  setUsers((prevUsers) => {
    const existingUser = prevUsers.find((user) => user?.name === trimmedUsername);
    if (existingUser) {
      return prevUsers;
    }
    return [...prevUsers, { name: trimmedUsername, color: "blue" }];
  });
  window.history.replaceState(null, "", `?username=${encodeURIComponent(trimmedUsername)}`);
};
...
if (!joinedUsername)
{
 ...
 <main className="flex items-center justify-center h-screen bg-gray-900">
  ...
  onSubmit={handleJoin}>
    ...
    />
    {joinError ? <p className="mb-4 text-sm text-red-400">{joinError}</p> : null}
    <button
      type="submit"
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="w-1/4 bg-gray-900 text-white border-r border-gray-700">
        ...
        {users.length === 0 ? (
          <div className="px-4 py-3 text-sm text-gray-400">No users joined yet.</div>
        ) : (
